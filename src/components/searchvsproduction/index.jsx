...
import './styles.scss'
import { CLOSE_IMG, GOLD_PHONE, ADD_IMG, BUTTON_REMOVE, VS_SOLID } from 'common/images'
...
class SearchProductionModal extends React.Component<Props, State> {
  ...
  onAddProductClick = (itemObject, productid, index) => {




    const itemTarget = document.getElementById("mini_item" + index)

    if (!itemTarget.classList.contains("add_item")) {
      this.setState({ isAddModalVisible: true, activeIndex: index })
      var buttonref = document.getElementById("button" + productid);
      var checkbuttonref = document.getElementById("checkedbutton" + productid);
      if (buttonref.classList.contains("checkedbutton")) {
        checkbuttonref.classList.add("checkedbutton");
      ...
      itemTarget.classList.add("add_item")
      ...
      this.props.dispatch({
        type: 'REMOVE_ITEM',
        payload: removeItem(itemObject.productId)
      })
      itemTarget.classList.remove("add_item")
    }
      ...
      <span>close</span>
        ...
        <div className="search_part">
          <div className={isBorderBottomHidden ? "search-cont border_bottom container" : "search-cont container"}>
            <input type="text" id="vs_input" autoFocus onChange={(e) => this.getProductByName(e.target.value)} placeholder="Type the product name here" onKeyDown={(e) => this.handleKeyDown(e)} />
            {
              isShortcutVisible && (
...

          {
            ...
            isShortcutVisible && (
              ...
              <SearchListForVs
                ...
                onRemoveProductClick={this.onRemoveProductClick}
              />
            )
          }
        ...
        <div className={count !== 0 && isAddModalVisible ? "add_item_part showitemmodal" : "add_item_part"}>
          <div className="add_title">
            ...
            }
            {
              count == 5 && (
                <div>
                  <p className="added_part">You have added <b>max number</b> of the product.</p>
                </div>
              )
            }
            {
                  ...
                  <li key={index}>
                    <div className="compare_titles">
                      <div className="image_outer">
                        <div className="image_size2" >
                          <img src={item.productImg || GOLD_PHONE} />
                        </div>
                      </div>
                      ...
                      <button type="button" className="remove_item" onClick={() => this.onItemRemove(item.productId)}><img src={BUTTON_REMOVE} /></button>
                    </div>
                  </li>
                )
              })
          ...
          {
            count > 1 && (
              <button className="btn btn-primary add_button" onClick={this.props.onVSButtonClick}><img src={VS_SOLID} /></button>
            )
          }
          {
            count == 1 && (
